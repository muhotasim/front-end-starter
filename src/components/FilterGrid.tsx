    ...
    return <form onSubmit={e=>{
        ...
        <div className="filter__grid">
            {grid.map((gridItem, index) => <div className="grid__item" key={index}>
                ...
                <Grid placeholder={gridItem.placeholder} value={gridValues[gridItem.effect_on]} condition={gridItem.condition} type={gridItem.type} effect_on={gridItem.effect_on} key={index} onChange={(val) => {
...
const Grid = ({ condition, type, effect_on, onChange, value, placeholder }: {  placeholder:string, value: any, condition: string, type: string, effect_on: string, onChange: (val: any) => void }) => {
    switch (type) {
        case "number":
