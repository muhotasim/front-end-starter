...
const LoginPage: React.FC = () => {
    ...
    const { isLoading, loggedIn, error } = useSelector((state: RootState) => state.users)
    ...
    useEffect(()=>{
        dispatch(userActions.updateState({error: null}));
    },[])
    if (loggedIn) {
        return <Navigate to={'/'} />
    }
    return <div className='page login-page'>
        <div className='login__container'>
            <h4 className='mb-15 section-title'><span className='mr-5'><span className='fa-icon fa fa-sign-in-alt' /></span> Login</h4>
            <div className='login-form px-15 animate-fade-in'>
                ...
                <div className='input-box mb-15'>
                    <button disabled={isLoading || !loginData.email || !loginData.password} className='btn btn-md btn-primary btn-block' onClick={() => { userActions.login(loginData.email, loginData.password)(dispatch) }}>Login <span className='ml-5'><span className={`fa-icon fa ${isLoading ? 'fa-spin fa-sync' : 'fa-arrow-right'}`} color='#fff'></span></span></button>
